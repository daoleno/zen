...
// openCodeTransportDigest is the digest of the bytes OpenCode's composer
// persists for a large Zen paste: the edge-trimmed payload plus one space.
func openCodeTransportDigest(payload string) string {
	return fmt.Sprintf("%x", sha256.Sum256([]byte(payload+" ")))
}

// TestOpenCodeLargePayloadInitialAdmissionMatchesTransportRepresentation
// reproduces the reported false mismatch at the watcher boundary with the
// corrected send-side model: the admission is bound to the exact bytes the owned
// composer persists (payload + one space), so provider evidence read verbatim by
// work's OpenCode reader resolves the turn instead of being reported as a byte
// mismatch. No reader normalization is involved.
func TestOpenCodeLargePayloadInitialAdmissionMatchesTransportRepresentation(t *testing.T) {
	io := newFakeSessionInputIO()
	ledger := newFakeTurnLedger()
	now := time.Now().UTC()
	payload := openCodeLargePastePayload("initial")
	digest := openCodeTransportDigest(payload)
	probe := &scriptedProviderActivityProbe{steps: []ProviderActivityObservation{
		{Structured: true, FallbackAllowed: true},
		{
...
// TestOpenCodeTransportAdmissionDuplicateRetryDoesNotReplay proves an exact
// retry under the same receipt reconciles the resolved transport admission and
// never queues a second provider mutation.
func TestOpenCodeTransportAdmissionDuplicateRetryDoesNotReplay(t *testing.T) {
	io := newFakeSessionInputIO()
	ledger := newFakeTurnLedger()
	now := time.Now().UTC()
	payload := openCodeLargePastePayload("initial")
	...
	probe := &scriptedProviderActivityProbe{steps: []ProviderActivityObservation{
		...
		{
			ID: "act-large", Status: "running", StartedAt: now.Add(time.Second),
			AdmissionStream: "opencode_db\x00ses_retry\x00/db", AdmissionID: "msg_retry",
			AdmissionCursor: 1, AdmissionAt: now.Add(time.Second), InputSHA256: digest,
			Structured: true,
		},
	}}
	w := lifecycleTestWatcher(io, ledger, probe)
	sessionID := "opencode-retry:@11"
	w.workers[sessionID] = &classifier.Worker{
		ID: sessionID, Command: "opencode", Cwd: "/repo/zen",
		PaneAlive: true, Delegated: true, State: classifier.StateUnknown,
	}
	turnID := sessionID + ":turn:1"
	if result, err := w.SubmitDelegatedInput(sessionID, payload, turnID, now); err != nil || result.Outcome != InputAccepted {
		t.Fatalf("first admission = (%+v, %v)", result, err)
	}
	result, err := w.SubmitDelegatedInput(sessionID, payload, turnID, now)
	if err != nil || !result.Duplicate || result.Outcome != InputAccepted || result.TurnID != turnID {
		t.Fatalf("exact retry = (%+v, %v), want duplicate accepted", result, err)
	}
	if len(io.queues) != 1 || len(io.submissions) != 1 {
		t.Fatalf("exact retry replayed input: queues=%d submissions=%d", len(io.queues), len(io.submissions))
	}
}

// TestOpenCodeAdmissionRejectsUntransformedAndDifferentDigests proves the
// admission is bound to the exact transport representation: neither the raw
// submitted bytes (the paste-summary-disabled shape) nor a genuinely different
// large payload may claim it. No false acceptance is possible.
func TestOpenCodeAdmissionRejectsUntransformedAndDifferentDigests(t *testing.T) {
	payload := openCodeLargePastePayload("initial")
	cases := []struct {
		name      string
		admission string
	}{
		{"raw untransformed bytes", fmt.Sprintf("%x", sha256.Sum256([]byte(payload)))},
		{"paste-summary-disabled shape", fmt.Sprintf("%x", sha256.Sum256([]byte(payload)))},
		{"genuinely different large payload", openCodeTransportDigest(openCodeLargePastePayload("different"))},
	}
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			...
			probe := &scriptedProviderActivityProbe{steps: []ProviderActivityObservation{
				{Structured: true, FallbackAllowed: true},
				{
					ID: "act-foreign", Status: "running", StartedAt: now.Add(time.Second),
					AdmissionStream: "opencode_db\x00ses_foreign\x00/db", AdmissionID: "msg_foreign",
					AdmissionCursor: 1, AdmissionAt: now.Add(time.Second), InputSHA256: test.admission,
					Structured: true,
				},
			}}
			w := lifecycleTestWatcher(io, ledger, probe)
			sessionID := "opencode-foreign:@12"
			w.workers[sessionID] = &classifier.Worker{
				ID: sessionID, Command: "opencode", Cwd: "/repo/zen",
				PaneAlive: true, Delegated: true, State: classifier.StateUnknown,
			}
			...
			result, err := w.SubmitDelegatedInput(sessionID, payload, turnID, now)
			if err == nil || result.Outcome != InputAmbiguous {
				t.Fatalf("foreign admission = (%+v, %v), want ambiguous", result, err)
			}
			if len(io.queues) != 1 {
				t.Fatalf("foreign admission replayed: queues=%d", len(io.queues))
			}
			if turn, hasTurn, _ := ledger.Turn(sessionID); hasTurn {
				t.Fatalf("foreign admission created a phantom Turn: %+v", turn)
			}
		})
	}
}

// TestOpenCodeNonInjectivePayloadRefusedBeforeMutation proves the send side
// refuses any payload the owned composer cannot represent byte-exactly
// (leading/trailing whitespace, tabs, or CR) before the tmux mutation. Such
// payloads would be trimmed or CRLF-normalized non-injectively, and are never
// silently accepted.
func TestOpenCodeNonInjectivePayloadRefusedBeforeMutation(t *testing.T) {
	base := openCodeLargePastePayload("initial")
	cases := []struct {
		name    string
		payload string
	}{
		{"leading spaces", "   " + base},
		{"leading tab", "\t" + base},
		{"double trailing spaces", base + "  "},
		{"newline then space", base + "\n "},
		{"trailing newline", base + "\n"},
		{"embedded CRLF", strings.Replace(base, "\n", "\r\n", 1)},
	}
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			io := newFakeSessionInputIO()
			ledger := newFakeTurnLedger()
			now := time.Now().UTC()
			w := lifecycleTestWatcher(io, ledger, &scriptedProviderActivityProbe{})
			sessionID := "opencode-refuse:@13"
			w.workers[sessionID] = &classifier.Worker{
				ID: sessionID, Command: "opencode", Cwd: "/repo/zen",
				PaneAlive: true, Delegated: true, State: classifier.StateUnknown,
			}
			turnID := sessionID + ":turn:1"
			result, err := w.SubmitDelegatedInput(sessionID, test.payload, turnID, now)
			if err == nil || result.Outcome != InputNotSubmitted {
				t.Fatalf("non-injective payload = (%+v, %v), want definitely not submitted", result, err)
			}
			if len(io.queues) != 0 || len(io.submissions) != 0 {
				t.Fatalf("refused payload reached tmux: queues=%d submissions=%d", len(io.queues), len(io.submissions))
			}
			if pendingList, _ := ledger.PendingInputAdmissions(sessionID); len(pendingList) != 0 {
				t.Fatalf("refused payload prepared a pending admission: %+v", pendingList)
			}
			if turn, hasTurn, _ := ledger.Turn(sessionID); hasTurn {
				t.Fatalf("refused payload created a Turn: %+v", turn)
			}
		})
	}
}

// TestProviderAdmissionPayloadDigestIsInjectiveOnAcceptedDomain guards the
// transport model directly: the large-paste append is injective, non-OpenCode
// providers hash the raw bytes, and the unsafe domain is refused.
func TestProviderAdmissionPayloadDigestIsInjectiveOnAcceptedDomain(t *testing.T) {
	first := openCodeLargePastePayload("first")
	second := openCodeLargePastePayload("second")
	firstDigest, err := ProviderAdmissionPayloadDigest("opencode --auto", first)
	if err != nil || firstDigest != openCodeTransportDigest(first) {
		t.Fatalf("opencode digest = %q err=%v, want transport digest", firstDigest, err)
	}
	secondDigest, err := ProviderAdmissionPayloadDigest("opencode --auto", second)
	if err != nil || firstDigest == secondDigest {
		t.Fatalf("distinct payloads shared a transport digest: %q vs %q err=%v", firstDigest, secondDigest, err)
	}
	rawDigest, err := ProviderAdmissionPayloadDigest("codex", first)
	if err != nil || rawDigest != fmt.Sprintf("%x", sha256.Sum256([]byte(first))) {
		t.Fatalf("codex digest = %q err=%v, want raw digest", rawDigest, err)
	}
	for _, unsafe := range []string{"   " + first, first + "  ", first + "\n", strings.Replace(first, "\n", "\r\n", 1)} {
		if _, err := ProviderAdmissionPayloadDigest("opencode", unsafe); err == nil {
			t.Fatalf("unsafe payload %q was accepted by the transport model", unsafe)
		}
	}
}
