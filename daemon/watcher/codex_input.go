...
func ensureCodexPaneInputConsumed(metadata codexTmuxPaneMetadata) error {
	...
	fd, err := unix.Open(metadata.paneTTY, unix.O_RDONLY|unix.O_NONBLOCK|unix.O_NOCTTY, 0)
	if err != nil {
		return fmt.Errorf("%w: inspect target pane PTY input: %v", errCodexMutationConflict, err)
	}
	defer unix.Close(fd)
