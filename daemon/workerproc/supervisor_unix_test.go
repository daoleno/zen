...
const supervisorHelperEnv = "ZEN_WORKERPROC_SUPERVISOR_HELPER"
...
func TestRunSupervisorOwnsAndReclaimsIsolatedSession(t *testing.T) {
	...
	command.Env = append(os.Environ(), supervisorHelperEnv+"=1", "ZEN_WORKERPROC_LEASE_DIR="+leaseDir)
...
func TestLargestLeaseRSS(t *testing.T) {
	...
	if got != "zen-worker-b" {
		t.Fatalf("largest = %q, want zen-worker-b (stable tie-break)", got)
	}
...
func TestParseAllMarkedProcessIDsGroupsByResource(t *testing.T) {
	out := []byte(strings.Join([]string{
		"10 /bin/sh ZEN_WORKER_RESOURCE_UNIT=zen-worker-a.scope HOME=/tmp",
		"11 /bin/sh ZEN_WORKER_RESOURCE_UNIT=zen-worker-b.scope",
		"12 /usr/bin/other",
		"13 /bin/tool ZEN_WORKER_RESOURCE_UNIT=zen-worker-a.scope PWD=/",
	}, "\n"))
	marked, err := parseAllMarkedProcessIDs(out)
	if err != nil {
		t.Fatal(err)
	}
	if !marked["zen-worker-a.scope"][10] || !marked["zen-worker-a.scope"][13] {
		t.Fatalf("resource a markers = %#v", marked["zen-worker-a.scope"])
	}
	if !marked["zen-worker-b.scope"][11] || marked["zen-worker-b.scope"][10] {
		t.Fatalf("resource b markers = %#v", marked["zen-worker-b.scope"])
	}
}
...
func TestRunSupervisorHelper(t *testing.T) {
	...
	resourceID := "zen-worker-helper-0123456789abcdef0123456789abcdef.scope"
	err := RunSupervisor(SupervisorConfig{
		ResourceID: resourceID,
		LeaseDir:   os.Getenv("ZEN_WORKERPROC_LEASE_DIR"),
		MemoryHigh: "1G",
		MemoryMax:  "2G",
		TasksMax:   64,
	...
	if matches, _ := filepath.Glob(filepath.Join(os.Getenv("ZEN_WORKERPROC_LEASE_DIR"), "*.json")); len(matches) != 0 {
