...
// openCodeLargePastePayload builds a delegated-shape prompt: several lines and
// well over 150 UTF-16 units, ending without whitespace the way
// delegatedLifecyclePayload's turn contract does.
func openCodeLargePastePayload(label string) string {
	return label + "\n\n" + strings.Repeat("context detail line\n", 12) + "run --turn-id turn:" + label + " --lease 300"
}

// TestOpenCodeAdmittedUserTextReversesOnlyLargePasteArtifact guards the exact
// reversal of OpenCode's composer rewrite: only a single trailing space on a
// summary-shaped paste is removed. Short and non-summary text (including a
// genuine trailing space) is left byte-identical so admission identity is never
// relaxed into generic whitespace tolerance.
func TestOpenCodeAdmittedUserTextReversesOnlyLargePasteArtifact(t *testing.T) {
	large := openCodeLargePastePayload("initial")
	astral := strings.Repeat("\U0001F600", 76) // 152 UTF-16 code units on one line
	ascii151 := strings.Repeat("a", 151)
	ascii150 := strings.Repeat("a", 150)
	tests := []struct{ name, persisted, want string }{
		{"large multiline artifact", large + " ", large},
		{"large multiline without artifact", large, large},
		{"astral above length threshold", astral + " ", astral},
		{"astral at length threshold", strings.Repeat("\U0001F600", 75) + " ", strings.Repeat("\U0001F600", 75) + " "},
		{"ascii above length threshold", ascii151 + " ", ascii151},
		{"ascii at length threshold", ascii150 + " ", ascii150 + " "},
		{"three short lines", "a\nb\nc ", "a\nb\nc"},
		{"two short lines keep genuine space", "line one\nline two ", "line one\nline two "},
		{"short genuine trailing space", "hello world ", "hello world "},
		{"empty", "", ""},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			if got := openCodeAdmittedUserText(test.persisted); got != test.want {
				t.Fatalf("admitted text = %q, want %q", got, test.want)
			}
		})
	}
}

// TestOpenCodeReaderReconstructsLargePasteInitialAndFollowUpAdmission runs the
// real SQLite projection over the exact bytes OpenCode persists for a large
// bracketed paste (trimmed paste plus the composer's single trailing space) and
// proves both the initial and follow-up turns hash to the submitted payload.
func TestOpenCodeReaderReconstructsLargePasteInitialAndFollowUpAdmission(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "opencode.db")
	started := time.Date(2026, 9, 10, 14, 0, 0, 0, time.UTC)
	initial := openCodeLargePastePayload("initial")
	followUp := openCodeLargePastePayload("follow-up")
	createOpenCodeFixtureDB(t, dbPath, []openCodeSessionSeed{
		{ID: "ses_paste", Directory: "/repo", CreatedMS: started.UnixMilli(), UpdatedMS: started.Add(2 * time.Minute).UnixMilli()},
	}, []openCodeMessageSeed{
		{ID: "msg_initial", SessionID: "ses_paste", CreatedMS: started.Add(time.Second).UnixMilli(), Data: `{"role":"user"}`},
		{ID: "msg_follow", SessionID: "ses_paste", CreatedMS: started.Add(time.Minute).UnixMilli(), Data: `{"role":"user"}`},
	}, []openCodePartSeed{
		{ID: "p_initial", MessageID: "msg_initial", SessionID: "ses_paste", CreatedMS: started.Add(time.Second).UnixMilli(), Data: mustOpenCodeTextPart(initial + " ")},
		{ID: "p_follow", MessageID: "msg_follow", SessionID: "ses_paste", CreatedMS: started.Add(time.Minute).UnixMilli(), Data: mustOpenCodeTextPart(followUp + " ")},
	})
	conversation, err := parseOpenCodeConversation(dbPath, "ses_paste")
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(byBody) != 2 {
		t.Fatalf("reconstructed user messages = %d, want 2: %#v", len(byBody), byBody)
	}
	for _, want := range []string{initial, followUp} {
		event, ok := byBody[want]
		if !ok {
			t.Fatalf("user message not reconstructed from large-paste bytes: %q", want)
		}
		digest := fmt.Sprintf("%x", sha256.Sum256([]byte(want)))
		if event.AdmissionSHA256 != digest {
			t.Fatalf("admission digest = %q, want %q", event.AdmissionSHA256, digest)
		}
	}
}

// TestOpenCodeReaderRejectsGenuinelyDifferentPasteContent proves the reversal
// does not collapse distinct prompts: two summary-shaped pastes that differ in
// content still produce different admitted text and admission digests.
func TestOpenCodeReaderRejectsGenuinelyDifferentPasteContent(t *testing.T) {
	submitted := openCodeLargePastePayload("initial")
	other := openCodeLargePastePayload("different") + "\nserve a different task"
	if openCodeAdmittedUserText(other+" ") == openCodeAdmittedUserText(submitted+" ") {
		t.Fatal("distinct large pastes collapsed to one admitted text")
	}
	submittedDigest := fmt.Sprintf("%x", sha256.Sum256([]byte(openCodeAdmittedUserText(submitted+" "))))
	otherDigest := fmt.Sprintf("%x", sha256.Sum256([]byte(openCodeAdmittedUserText(other+" "))))
	if submittedDigest == otherDigest {
		t.Fatal("distinct large pastes shared an admission digest")
	}
}
