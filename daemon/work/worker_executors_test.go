...
func TestWorkerExecutorInfersProviderRuntimeAndCapabilities(t *testing.T) {
	...
	codex, ok := cfg.WorkerExecutor("codex")
	if !ok {
		t.Fatal("codex executor missing")
	}
	if codex.Provider != "codex" || codex.Runtime != WorkerRuntimeTmux {
	...
	claude, ok := cfg.WorkerExecutor("claude")
	if !ok {
	...
	cursor, ok := cfg.WorkerExecutor("agent")
	if !ok {
		t.Fatal("agent executor missing")
	}
	if cursor.Provider != WorkerProviderCursor || !cursor.Capabilities.InteractiveTTY || !cursor.Capabilities.StructuredEvents {
	...
	other, ok := cfg.WorkerExecutor("other")
	if !ok {
		t.Fatal("other executor missing")
	}
	if other.Provider != WorkerProviderCustom || !other.Capabilities.InteractiveTTY {
...
func TestInferAgentProviderRecognizesCursorAgent(t *testing.T) {
	for _, value := range []string{
		...
		if got := InferWorkerProvider(value); got != WorkerProviderCursor {
			t.Fatalf("InferAgentProvider(%q) = %q, want %q", value, got, WorkerProviderCursor)
		}
	}
	if got := InferWorkerProvider("agent"); got != "" {
...
func TestProfileClientExecutorSeparatesAliasFromCanonicalClient(t *testing.T) {
	cases := []struct {
		...
		{name: "codex alias id", in: []string{"primary", "codex --flag"}, want: WorkerProviderCodex},
		{name: "codex kind", in: []string{"codex", "env X=1 -- mybin"}, want: WorkerProviderCodex},
		{name: "claude wrapper", in: []string{"desk", "claude --permission-mode bypassPermissions"}, want: WorkerProviderClaude},
