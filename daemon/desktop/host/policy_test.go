...
func TestUnattendedAuthorization(t *testing.T) {
	tests := []struct {
		...
		{"trusted-approved-plain", func(_ *Gate, r *Request, _ *Session) { r.TLS = false; r.LANApproved = true }, true},
		{"plain-unapproved", func(_ *Gate, r *Request, _ *Session) { r.TLS = false; r.LANApproved = false }, false},
