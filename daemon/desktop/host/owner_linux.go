...
// SYSTEM owner contract: the unattended desktop broker admits members of the
// enrolled system unit's cgroup carrying the enrolled UID. The real owner is
// a watcher-spawned daemon child (zen-dev starts the daemon with
// exec.Command/Start and rebuilds it in place), so peerPID == MainPID must
// NOT be required: it would reject every legitimate rebuild while adding
// nothing over the cgroup proof. The unit's current MainPID only proves the
// unit is active. A user-manager unit is never canonical, and an unrelated
...
// verifyCanonicalOwner is the pure SYSTEM owner decision: the kernel peer
