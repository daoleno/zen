...
func TestRecentWorkResultEventsAreBoundedDeterministicAndPersistAfterRead(t *testing.T) {
	...
	second, err := store.CreateWork(Work{
		...
		Objective:        "Prove the recent bound across Works.",
		Status:           WorkRunning,
		OwnerSessionID:   "brain-agent-cards:@2",
		CompletionPolicy: CompletionBounded,
		NextAction:       "Review the second result.",
	})
	if err != nil {
		t.Fatal(err)
	}
	appendEvent := func(id, workID, kind, payload string, offset time.Duration) {
		t.Helper()
		now = now.Add(offset)
		if _, _, err := store.AppendWorkEvent(WorkEvent{
			ID: id, WorkID: workID, Kind: kind, DedupeKey: id,
			PayloadRef: payload, Actionable: true,
		}); err != nil {
			t.Fatal(err)
		}
	}
	appendEvent("event-progress", first.ID, "session.progress", "session:brain-agent-cards:@1", time.Minute)
	appendEvent("event-b", first.ID, "session.done", "session:brain-agent-closed:@2", time.Minute)
	appendEvent("event-calendar", first.ID, "calendar.failure", "calendar-result-id", 0)
	appendEvent("event-c", first.ID, "session.failed", "session:brain-agent-cards:@1", time.Minute)
	appendEvent("event-d", second.ID, "session.done", "session:brain-agent-cards:@2", time.Minute)

	events, err := store.RecentWorkResultEvents(2)
	if err != nil {
		t.Fatal(err)
	}
	// Latest-per-Work collapses first Work to event-c; bound keeps the two latest Works.
	if len(events) != 2 || events[0].EventID != "event-c" || events[1].EventID != "event-d" {
		t.Fatalf("bounded latest-per-work result events = %#v", events)
	}
	if events[0].SessionID != "brain-agent-cards:@1" ||
		events[0].Summary != first.NextAction || !events[0].Unread {
	...
	if err := store.MarkWorkRead(first.ID); err != nil {
		t.Fatal(err)
	}
	...
	if events[0].Unread || !events[1].Unread {
		t.Fatalf("read state not projected: %#v", events)
	}
}

func TestRecentWorkResultEventsProjectCurrentPresentableOutcomePerWork(t *testing.T) {
	store, err := NewStore(t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	...
	claimAndConsume := func(eventID string) {
		t.Helper()
		if _, ok, err := store.ClaimNextActionableEvent(hostID); err != nil || !ok {
			t.Fatalf("claim %s: ok=%v err=%v", eventID, ok, err)
		}
		if _, _, err := store.ConsumeClaimedWorkEvent(eventID, hostID); err != nil {
			t.Fatalf("consume %s: %v", eventID, err)
		}
	}

	// Live-shaped closed commitment: REJECT then ACCEPT, then Work marked done
	// after both Events (CreatedAt < TerminalAt). Neither card may replay.
	acceptance, err := store.CreateWork(Work{
		Title:            "zen-pi-opencode-first-class-acceptance",
		Objective:        "Close the acceptance commitment.",
		Status:           WorkWaiting,
		CompletionPolicy: CompletionBounded,
	...
	now = time.Date(2026, 8, 5, 20, 52, 48, 0, time.UTC)
	reject, _, err := store.AppendWorkEvent(WorkEvent{
		ID: "645a5a2a-reject", WorkID: acceptance.ID, Kind: "session.done",
		DedupeKey: "hist-reject", Summary: "Round-2 REJECT: P0-1 hash mismatch remains; prior code findings closed.",
		Actionable: true,
	})
	if err != nil {
		t.Fatal(err)
	}
	now = time.Date(2026, 8, 5, 20, 54, 4, 0, time.UTC)
	accept, _, err := store.AppendWorkEvent(WorkEvent{
		ID: "1a6ddd99-accept", WorkID: acceptance.ID, Kind: "session.done",
		DedupeKey: "hist-accept", Summary: "Round-2 ACCEPT P0=0 P1=0 P2=0; identity 0c222480 confirmed twice.",
		Actionable: true,
	})
	if err != nil {
		t.Fatal(err)
	}
	claimAndConsume(reject.ID)
	claimAndConsume(accept.ID)
	now = time.Date(2026, 8, 5, 21, 24, 19, 0, time.UTC)
	done := WorkDone
	if _, err := store.UpdateWork(acceptance.ID, WorkUpdate{Status: &done}); err != nil {
		t.Fatal(err)
	}

	support, err := store.CreateWork(Work{
		Title:            "zen-pi-opencode-first-class-support",
		Objective:        "Ship first-class support.",
		Status:           WorkWaiting,
		CompletionPolicy: CompletionBounded,
		NextAction:       "Review the delegated Session result.",
	})
	...
	now = time.Date(2026, 8, 5, 21, 22, 13, 0, time.UTC)
	published, _, err := store.AppendWorkEvent(WorkEvent{
		ID: "8c2cbfdc-published", WorkID: support.ID, Kind: "session.done",
		DedupeKey: "support-published", Summary: "v0.1.0-beta.11 published; feature+release commits on main.",
		Actionable: true,
	})
	if err != nil {
		t.Fatal(err)
	}
	claimAndConsume(published.ID)
	now = time.Date(2026, 8, 5, 21, 24, 8, 0, time.UTC)
	if _, err := store.UpdateWork(support.ID, WorkUpdate{Status: &done}); err != nil {
		t.Fatal(err)
	}

	// Stranded waiting Work: newer ACCEPT supersedes REJECT without ReadAt or terminalization.
	stranded, err := store.CreateWork(Work{
		Title:            "zen-stranded-waiting-history",
		Objective:        "Keep only the current presentable outcome.",
		Status:           WorkWaiting,
		CompletionPolicy: CompletionBounded,
		NextAction:       "Review the delegated Session result.",
	})
	...
	now = now.Add(time.Minute)
	strandedReject, _, err := store.AppendWorkEvent(WorkEvent{
		ID: "stranded-reject", WorkID: stranded.ID, Kind: "session.done",
		DedupeKey: "stranded-reject", Summary: "Round-1 REJECT",
		Actionable: true,
	})
	if err != nil {
		t.Fatal(err)
	...
	strandedAccept, _, err := store.AppendWorkEvent(WorkEvent{
		ID: "stranded-accept", WorkID: stranded.ID, Kind: "session.done",
		DedupeKey: "stranded-accept", Summary: "Round-1 ACCEPT",
		Actionable: true,
	})
	if err != nil {
		t.Fatal(err)
	}
	claimAndConsume(strandedReject.ID)
	claimAndConsume(strandedAccept.ID)

	// Current waiting Work: done then needs_input — only latest remains visible.
	current, err := store.CreateWork(Work{
		Title:            "zen-manual-input-and-brand-icons",
		Objective:        "Keep the live commitment visible.",
		Status:           WorkWaiting,
		CompletionPolicy: CompletionBounded,
		NextAction:       "Resolve the delegated Session request.",
	})
	if err != nil {
		t.Fatal(err)
	}
	now = time.Date(2026, 8, 6, 2, 9, 18, 0, time.UTC)
	completed, _, err := store.AppendWorkEvent(WorkEvent{
		ID: "current-done", WorkID: current.ID, Kind: "session.done",
		DedupeKey: "current-done", Summary: "Delegated turn completed",
		Actionable: true,
	})
	if err != nil {
		t.Fatal(err)
	}
	now = time.Date(2026, 8, 6, 2, 19, 33, 0, time.UTC)
	needsInput, _, err := store.AppendWorkEvent(WorkEvent{
		ID: "current-needs-input", WorkID: current.ID, Kind: "session.needs_input",
		DedupeKey: "current-needs-input", Summary: "go vet ./... ; echo VET_EXIT:$?",
		Actionable: true,
	})
	if err != nil {
		t.Fatal(err)
	}
	claimAndConsume(completed.ID)
	claimAndConsume(needsInput.ID)

	// Post-terminal equality remains presentable (Calendar-style update-then-append).
	calendarish, err := store.CreateWork(Work{
		Title:            "zen-terminal-equal-time-result",
		Objective:        "Keep a terminal result created at TerminalAt.",
		Status:           WorkWaiting,
		CompletionPolicy: CompletionBounded,
		NextAction:       "Review the terminal result.",
	})
	if err != nil {
		t.Fatal(err)
	}
	now = time.Date(2026, 8, 6, 3, 0, 0, 0, time.UTC)
	if _, err := store.UpdateWork(calendarish.ID, WorkUpdate{Status: &done}); err != nil {
		t.Fatal(err)
	}
	terminalResult, _, err := store.AppendWorkEvent(WorkEvent{
		ID: "terminal-equal-done", WorkID: calendarish.ID, Kind: "session.done",
		DedupeKey: "terminal-equal-done", Summary: "Terminal result at TerminalAt",
		Actionable: true,
	})
	if err != nil {
		t.Fatal(err)
	}
	claimAndConsume(terminalResult.ID)

	// Post-terminal bookkeeping must not hide the latest terminal card.
	now = time.Date(2026, 8, 6, 3, 5, 0, 0, time.UTC)
	title := "zen-terminal-equal-time-result (retitled)"
	next := "Bookkeeping only."
	owner := "brain-agent-owner-bookkeeping:@1"
	contextRef := "worklog:bookkeeping"
	calendarish, err = store.UpdateWork(calendarish.ID, WorkUpdate{
		Title:          &title,
		NextAction:     &next,
	...
	if calendarish.TerminalAt == nil || !calendarish.TerminalAt.Equal(time.Date(2026, 8, 6, 3, 0, 0, 0, time.UTC)) {
		t.Fatalf("TerminalAt mutated by bookkeeping: %#v", calendarish)
	}
	if !calendarish.UpdatedAt.After(*calendarish.TerminalAt) {
		t.Fatalf("Expected UpdatedAt after TerminalAt after bookkeeping: %#v", calendarish)
	}

	// Before-fix shape would still flood; after reload only current presentable cards remain.
	reloaded, err := NewStore(store.Root)
	if err != nil {
		t.Fatal(err)
	}
	events, err := reloaded.RecentWorkResultEvents(20)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(events) != 3 ||
		events[0].EventID != strandedAccept.ID ||
		events[1].EventID != needsInput.ID ||
		events[2].EventID != terminalResult.ID {
		t.Fatalf("current presentable projection after refresh = %#v", events)
	}
	for _, banned := range []string{reject.ID, accept.ID, published.ID, strandedReject.ID, completed.ID} {
		for _, id := range ids {
			if id == banned {
				t.Fatalf("non-presentable card replayed %s in %#v", banned, ids)
			}
		}
	}
	if !events[0].Unread || !events[1].Unread || !events[2].Unread {
		t.Fatalf("unread emphasis lost: %#v", events)
	}
	if events[2].WorkTitle != title {
		t.Fatalf("latest terminal card lost retitled Work projection: %#v", events[2])
	}

	claimed, err := reloaded.ClaimedActionableEvents()
	if err != nil {
		t.Fatal(err)
	}
	if len(claimed) != 0 {
		t.Fatalf("exact-once delivery lost after projection change: %#v", claimed)
	}
	for _, workID := range []string{acceptance.ID, support.ID, stranded.ID, current.ID, calendarish.ID} {
		stored, err := reloaded.ListWorkEvents(workID)
		if err != nil {
			t.Fatal(err)
		}
		for _, event := range stored {
			if event.ConsumedAt == nil {
				t.Fatalf("durable consumed fact rewritten for %s: %#v", workID, stored)
			}
		}
	}
}

func TestOrchestrationSchemaV3BackfillsTerminalAtFromUpdatedAtOnce(t *testing.T) {
	root := t.TempDir()
	path := filepath.Join(root, "state", "orchestration.json")
	if err := os.MkdirAll(filepath.Dir(path), 0o700); err != nil {
		t.Fatal(err)
	}
	created := time.Date(2026, 8, 5, 20, 30, 58, 0, time.UTC)
	rejectAt := time.Date(2026, 8, 5, 20, 52, 48, 0, time.UTC)
	acceptAt := time.Date(2026, 8, 5, 20, 54, 4, 0, time.UTC)
	terminalAt := time.Date(2026, 8, 5, 21, 24, 19, 0, time.UTC)
	supportEventAt := time.Date(2026, 8, 5, 21, 22, 13, 0, time.UTC)
	supportTerminalAt := time.Date(2026, 8, 5, 21, 24, 8, 0, time.UTC)
	legacy := map[string]any{
		"schema_version": 3,
		"migrations":     map[string]any{},
		"brain_work": []map[string]any{
			{
				"work_id":           "4146c81a-acceptance",
				"title":             "zen-pi-opencode-first-class-acceptance",
				"objective":         "Close the acceptance commitment.",
				"status":            "done",
				"completion_policy": "bounded",
				"created_at":        created.Format(time.RFC3339Nano),
				"updated_at":        terminalAt.Format(time.RFC3339Nano),
			},
			{
				"work_id":           "485cf61e-support",
				"title":             "zen-pi-opencode-first-class-support",
				"objective":         "Ship first-class support.",
				"status":            "done",
				"completion_policy": "bounded",
				"created_at":        created.Format(time.RFC3339Nano),
				"updated_at":        supportTerminalAt.Format(time.RFC3339Nano),
			},
			{
				"work_id":           "waiting-open",
				"title":             "still-open",
				"objective":         "Stay non-terminal.",
				"status":            "waiting",
				"completion_policy": "bounded",
				"created_at":        created.Format(time.RFC3339Nano),
				"updated_at":        acceptAt.Format(time.RFC3339Nano),
			},
		},
		"brain_work_events": []map[string]any{
			{
				"event_id": "645a5a2a-reject", "work_id": "4146c81a-acceptance",
				"kind": "session.done", "dedupe_key": "hist-reject",
				"summary": "REJECT", "actionable": true,
				"created_at":               rejectAt.Format(time.RFC3339Nano),
				"claimed_at":               rejectAt.Format(time.RFC3339Nano),
				"delivery_host_session_id": "brain-agent-host:@1",
				"consumed_at":              rejectAt.Add(time.Second).Format(time.RFC3339Nano),
			},
			{
				"event_id": "1a6ddd99-accept", "work_id": "4146c81a-acceptance",
				"kind": "session.done", "dedupe_key": "hist-accept",
				"summary": "ACCEPT", "actionable": true,
				"created_at":               acceptAt.Format(time.RFC3339Nano),
				"claimed_at":               acceptAt.Format(time.RFC3339Nano),
				"delivery_host_session_id": "brain-agent-host:@1",
				"consumed_at":              acceptAt.Add(time.Second).Format(time.RFC3339Nano),
			},
			{
				"event_id": "8c2cbfdc-published", "work_id": "485cf61e-support",
				"kind": "session.done", "dedupe_key": "support-published",
				"summary": "published", "actionable": true,
				"created_at":               supportEventAt.Format(time.RFC3339Nano),
				"claimed_at":               supportEventAt.Format(time.RFC3339Nano),
				"delivery_host_session_id": "brain-agent-host:@1",
				"consumed_at":              supportEventAt.Add(time.Second).Format(time.RFC3339Nano),
			},
		},
	}
	raw, err := json.Marshal(legacy)
	...
	acceptance, err := store.Work("4146c81a-acceptance")
	if err != nil {
		t.Fatal(err)
	}
	if acceptance.TerminalAt == nil || !acceptance.TerminalAt.Equal(terminalAt) {
		t.Fatalf("acceptance TerminalAt backfill = %#v want %s", acceptance.TerminalAt, terminalAt)
	}
	support, err := store.Work("485cf61e-support")
	if err != nil {
		t.Fatal(err)
	}
	if support.TerminalAt == nil || !support.TerminalAt.Equal(supportTerminalAt) {
		t.Fatalf("support TerminalAt backfill = %#v want %s", support.TerminalAt, supportTerminalAt)
	}
	waiting, err := store.Work("waiting-open")
	if err != nil {
		t.Fatal(err)
	}
	if waiting.TerminalAt != nil {
		t.Fatalf("non-terminal Work gained TerminalAt: %#v", waiting)
	}

	events, err := store.RecentWorkResultEvents(20)
	if err != nil {
		t.Fatal(err)
	}
	if len(events) != 0 {
		t.Fatalf("cited live backlog still presentable after migration: %#v", events)
	}
	...
	database, migrated, err := decodeOrchestrationDatabase(persisted)
	if err != nil {
		t.Fatal(err)
	}
	if migrated || database.SchemaVersion != orchestrationSchemaVersion {
		t.Fatalf("persisted migration incomplete: schema=%d migrated=%v", database.SchemaVersion, migrated)
	}

	// Later bookkeeping after migration must not reinterpret UpdatedAt as the boundary.
	now := terminalAt.Add(2 * time.Hour)
	store.now = func() time.Time { return now }
	title := "acceptance retitled after migration"
	acceptance, err = store.UpdateWork(acceptance.ID, WorkUpdate{Title: &title})
	if err != nil {
		t.Fatal(err)
	}
	if !acceptance.UpdatedAt.Equal(now) || acceptance.TerminalAt == nil || !acceptance.TerminalAt.Equal(terminalAt) {
		t.Fatalf("migration TerminalAt reinterpreted via UpdatedAt: %#v", acceptance)
	}
	events, err = store.RecentWorkResultEvents(20)
	if err != nil {
		t.Fatal(err)
	...
	if len(events) != 0 {
		t.Fatalf("post-migration bookkeeping revived backlog cards: %#v", events)
	}
	...
	acceptance, err = reloaded.Work(acceptance.ID)
	if err != nil {
		t.Fatal(err)
	}
	if acceptance.TerminalAt == nil || !acceptance.TerminalAt.Equal(terminalAt) {
		t.Fatalf("reload lost migrated TerminalAt: %#v", acceptance)
	}
	// Ensure second open did not rewrite identity besides the intentional bookkeeping persist.
	if !bytes.Contains(second, []byte(`"schema_version": 4`)) && !bytes.Contains(second, []byte(`"schema_version":4`)) {
		t.Fatalf("schema 4 not persisted:\n%s", second)
	}
}

func TestWorkTerminalAtClearsOnReopenAndResetsOnReterminalize(t *testing.T) {
	store, err := NewStore(t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	...
	item, err := store.CreateWork(Work{
		Title:            "reopenable commitment",
		Objective:        "Allow UpdateWork reopen of terminal Work.",
		Status:           WorkWaiting,
		CompletionPolicy: CompletionBounded,
	})
	if err != nil {
	...
	now = now.Add(time.Minute)
	historical, _, err := store.AppendWorkEvent(WorkEvent{
		ID: "reopen-historical", WorkID: item.ID, Kind: "session.done",
		DedupeKey: "reopen-historical", Summary: "First terminal result",
		Actionable: true,
	})
	if err != nil {
		t.Fatal(err)
	...
	if _, ok, err := store.ClaimNextActionableEvent(hostID); err != nil || !ok {
		t.Fatalf("claim historical: ok=%v err=%v", ok, err)
	}
	if _, _, err := store.ConsumeClaimedWorkEvent(historical.ID, hostID); err != nil {
		t.Fatal(err)
	}
	...
	if item.TerminalAt == nil || !item.TerminalAt.Equal(firstTerminalAt) {
		t.Fatalf("first TerminalAt = %#v want %s", item.TerminalAt, firstTerminalAt)
	}
	events, err := store.RecentWorkResultEvents(10)
	if err != nil {
		t.Fatal(err)
	...
	if len(events) != 0 {
		t.Fatalf("pre-terminal historical result remained presentable: %#v", events)
	}

	// UpdateWork allows reopen to a non-terminal status; clear TerminalAt.
	now = now.Add(time.Minute)
	waiting := WorkWaiting
	item, err = store.UpdateWork(item.ID, WorkUpdate{Status: &waiting})
	if err != nil {
		t.Fatal(err)
	}
	if item.TerminalAt != nil {
		t.Fatalf("reopened Work retained TerminalAt: %#v", item)
	}
	events, err = store.RecentWorkResultEvents(10)
	if err != nil {
		t.Fatal(err)
	}
	if len(events) != 1 || events[0].EventID != historical.ID {
		t.Fatalf("reopened Work should project latest presentable result: %#v", events)
	}

	now = now.Add(time.Minute)
	latest, _, err := store.AppendWorkEvent(WorkEvent{
		ID: "reopen-latest", WorkID: item.ID, Kind: "session.done",
		DedupeKey: "reopen-latest", Summary: "Second result after reopen",
		Actionable: true,
	})
	if err != nil {
		t.Fatal(err)
	...
	if _, ok, err := store.ClaimNextActionableEvent(hostID); err != nil || !ok {
		t.Fatalf("claim latest: ok=%v err=%v", ok, err)
	}
	if _, _, err := store.ConsumeClaimedWorkEvent(latest.ID, hostID); err != nil {
		t.Fatal(err)
	}
	events, err = store.RecentWorkResultEvents(10)
	if err != nil {
	...
	if len(events) != 1 || events[0].EventID != latest.ID {
		t.Fatalf("latest-per-Work after reopen failed: %#v", events)
	}

	now = now.Add(time.Minute)
	secondTerminalAt := now
	item, err = store.UpdateWork(item.ID, WorkUpdate{Status: &done})
	if err != nil {
		t.Fatal(err)
	}
	if item.TerminalAt == nil || !item.TerminalAt.Equal(secondTerminalAt) {
		t.Fatalf("reterminalize TerminalAt = %#v want %s", item.TerminalAt, secondTerminalAt)
	}
	if item.TerminalAt.Equal(firstTerminalAt) {
		t.Fatal("reterminalize reused the first TerminalAt stamp")
	}
	events, err = store.RecentWorkResultEvents(10)
	if err != nil {
		t.Fatal(err)
	...
	if len(events) != 0 {
		t.Fatalf("reterminalize left pre-boundary results presentable: %#v", events)
	}

	// Equality at the new TerminalAt remains presentable.
	equal, _, err := store.AppendWorkEvent(WorkEvent{
		ID: "reopen-equal", WorkID: item.ID, Kind: "session.done",
		DedupeKey: "reopen-equal", Summary: "Result at new TerminalAt",
		Actionable: true,
	})
	if err != nil {
		t.Fatal(err)
	...
	if _, ok, err := store.ClaimNextActionableEvent(hostID); err != nil || !ok {
		t.Fatalf("claim equal: ok=%v err=%v", ok, err)
	}
	if _, _, err := store.ConsumeClaimedWorkEvent(equal.ID, hostID); err != nil {
		t.Fatal(err)
	}
	events, err = store.RecentWorkResultEvents(10)
	if err != nil {
	...
	if len(events) != 1 || events[0].EventID != equal.ID {
		t.Fatalf("equality at reterminalize boundary missing: %#v", events)
	}
}
...
func TestRecentWorkResultEventsPreserveOccurrenceFactsAcrossWorkMutationAndReload(t *testing.T) {
	...
	// Latest-per-Work keeps only the current occurrence; its captured summary/source
	// must survive later Work mutation and store reload.
	if len(events) != 1 ||
		events[0].EventID != "event-second" ||
		events[0].Summary != "Second occurrence completed the App projection." ||
		events[0].SessionName != "App worker" ||
		events[0].Summary == mutatedNext ||
		events[0].Summary == mutatedObjective {
		t.Fatalf("immutable current occurrence facts = %#v", events)
	}
}
