...
func TestWorkerSessionWireAdvertisesKnownStructuredProvider(t *testing.T) {
	...
	for _, command := range []string{"codex", "pi --session /tmp/x.jsonl", "opencode --auto"} {
		wire := srv.workerSessionWire(&classifier.Worker{ID: command + "-1", Command: command})
...
func TestWorkerSessionWireUsesConfiguredExecutorCapabilityForGenericCommand(t *testing.T) {
	srv := &Server{execs: &work.ExecutorConfig{ByName: map[string]work.Executor{
		"future": {
			Name:    "future",
			Command: "future-agent --structured",
			Kind:    work.WorkerProviderGrok,
		},
	}}}
	worker := &classifier.Worker{ID: "future-1", Command: "/opt/bin/future-agent"}
	wire := srv.workerSessionWire(worker)
	if wire == nil || !wire.Capabilities.StructuredEvents {
		t.Fatalf("configured generic capabilities = %#v", wire)
	}
	provider := srv.structuredProviderForWorker(worker)
	if provider != work.WorkerProviderGrok {
		t.Fatalf("configured provider = %q, want grok", provider)
	}
	conversation, err := work.NewProviderConversationReader().Load(*worker, provider, time.Now())
	if err != nil {
		t.Fatal(err)
	}
...
func TestWorkerSessionWireDoesNotAdvertisePlainShell(t *testing.T) {
	srv := &Server{execs: &work.ExecutorConfig{ByName: map[string]work.Executor{
		"future": {
			Name:    "future",
			Command: "future-agent --structured",
	...
	wire := srv.workerSessionWire(&classifier.Worker{ID: "shell-1", Command: "zsh"})
	if wire == nil || wire.Capabilities.StructuredEvents {
		t.Fatalf("plain shell capabilities = %#v", wire)
	}
}

func TestWorkerSessionWireDoesNotInferStructuredProviderFromShellTitle(t *testing.T) {
	srv := &Server{execs: &work.ExecutorConfig{ByName: map[string]work.Executor{
		"codex": {
			Name:    "codex",
			Command: "codex",
			Kind:    work.WorkerProviderCodex,
		},
	}}}
	for _, name := range []string{"Codex notes", "Claude research", "Cursor Agent scratch", "Grok shell"} {
		wire := srv.workerSessionWire(&classifier.Worker{
			ID:      "shell-title",
			Name:    name,
			Command: "zsh",
		})
...
func TestWorkerSessionWireModelProfileCapabilitiesFromRouteTable(t *testing.T) {
	root := t.TempDir()
	...
	bypass := srv.workerSessionWire(&classifier.Worker{ID: "shell:@1", Name: "Codex", Command: "codex"})
	...
	routed := srv.workerSessionWire(&classifier.Worker{ID: "tmux:@routed", Name: "zsh notes", Command: "zsh"})
	...
	liveRouted := liveSrv.workerSessionWire(&classifier.Worker{ID: "tmux:@live-routed", Name: "zsh notes", Command: "zsh"})
	...
	native := srv.workerSessionWire(&classifier.Worker{ID: "tmux:@native", Name: "Codex", Command: "codex"})
	...
	list := srv.workerSessionsWire([]*classifier.Worker{
	...
	restarted := srv2.workerSessionWire(&classifier.Worker{ID: "tmux:@routed", Command: "zsh"})
	...
	restartNative := srv2.workerSessionWire(&classifier.Worker{ID: "tmux:@native", Command: "codex"})
