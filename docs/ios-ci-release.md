...
The tracked general/Android prerelease is `0.1.0-beta.8`. iOS derives the App Store-valid numeric marketing version `0.1.0` from that value and writes it to both `CFBundleShortVersionString` and every generated Xcode `MARKETING_VERSION`. Its reviewed monotonic baseline is build `9` in `app/ios-build.json`, independent of Android `versionCode` because App Store Connect history may be ahead. Release preparation must increment that integer above every build already uploaded for the same Apple app and marketing version before creating the next tag. The required positive manual `build_number` overrides it for recovery/testing and is written to both `CFBundleVersion` and every generated `CURRENT_PROJECT_VERSION`. IPA verification and the release manifest use the resolved packaged iOS value.
...
7. After the prepared commit containing the reviewed app version, Android `versionCode`, independent unused iOS build number, and matching release notes is pushed to `main`, create and push the annotated tag. That one push automatically builds Preview with the tracked iOS build number and publishes it to TestFlight. For manual testing or recovery, dispatch `iOS signed release` with `app_identity=preview`, an explicit `ref`, an unused positive `build_number` for the Preview app/version, and the intended destination. Production remains manual.
