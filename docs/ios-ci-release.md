...
The tracked general/Android prerelease is `0.1.0-beta.10`. iOS derives the App Store-valid numeric marketing version `0.1.0` from that value and writes it to both `CFBundleShortVersionString` and every generated Xcode `MARKETING_VERSION`. Its reviewed monotonic baseline is build `11` in `app/ios-build.json`, independent of Android `versionCode` because App Store Connect history may be ahead. Release preparation must increment that integer above every build already uploaded for the same Apple app and marketing version before creating the next tag. The required positive manual `build_number` overrides it for recovery/testing and is written to both `CFBundleVersion` and every generated `CURRENT_PROJECT_VERSION`. IPA verification and the release manifest use the resolved packaged iOS value.
