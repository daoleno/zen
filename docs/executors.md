...
- Each delegated session receives a daemon-namespaced resource ID, a durable lease under `~/.zen/run/worker-resources`, and a short private temporary directory under `~/.zen/t/<6-char digest>` (URL-safe digest of the full resource unit, with a mode-0600 ownership marker naming that unit). `TMPDIR`, `TMP`, `TEMP`, and `ZEN_BUILD_TMPDIR` point at that directory so AF_UNIX paths stay within platform limits. Zen removes it only after the exact owned session is stopped and the marker still matches. Release/Reconcile also reclaim leftover legacy directories under `~/.zen/tmp/worker-resources/<owner>/<full-unit>` for sessions launched before the short layout; live legacy directories are left alone until their session ends.
- Delegated working directories under volatile or memory-backed temporary roots such as `/tmp`, `/private/tmp`, `/var/tmp`, `/run/user`, and `/dev/shm` are rejected. Edit the supplied repository directly by default. Brain receives `ZEN_WORKTREE_ROOT=~/.zen/worktrees` for exceptions: explicit user request, concrete conflicting edits, or justified necessary isolation. Briefly explain the actual reason; delegation or a dirty repository alone is not a reason to create one. Worktree use still requires integration into the owning target repository and requested delivery.
