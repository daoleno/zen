...
#   ./scripts/verify-release-identity.sh --tag v0.1.0-beta.17
#   ./scripts/verify-release-identity.sh --stage dist-download/v0.1.0-beta.17
...
EXPECTED_VERSION="0.1.0-beta.17"
...
if (config.version !== '0.1.0-beta.17') {
  throw new Error(`general/Android version must remain 0.1.0-beta.17; got ${config.version}`);
}
