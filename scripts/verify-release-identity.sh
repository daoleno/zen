...
#   ./scripts/verify-release-identity.sh --tag v0.1.0-beta.15
#   ./scripts/verify-release-identity.sh --stage dist-download/v0.1.0-beta.15
...
EXPECTED_VERSION="0.1.0-beta.15"
...
if (config.version !== '0.1.0-beta.15') {
  throw new Error(`general/Android version must remain 0.1.0-beta.15; got ${config.version}`);
}
