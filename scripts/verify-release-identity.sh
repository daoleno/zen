...
#   ./scripts/verify-release-identity.sh --stage dist-download/v0.1.0-beta.4
...
if (config.version !== '0.1.0-beta.4') {
  throw new Error(`general/Android version must remain 0.1.0-beta.4; got ${config.version}`);
}
...
if (config.ios.infoPlist.CFBundleVersion !== '4') {
  throw new Error(`iOS build number must resolve to 4; got ${config.ios.infoPlist.CFBundleVersion}`);
}
