...
#   ./scripts/verify-release-identity.sh --tag v0.1.0-beta.16
#   ./scripts/verify-release-identity.sh --stage dist-download/v0.1.0-beta.16
...
EXPECTED_VERSION="0.1.0-beta.16"
...
if (config.version !== '0.1.0-beta.16') {
  throw new Error(`general/Android version must remain 0.1.0-beta.16; got ${config.version}`);
}
