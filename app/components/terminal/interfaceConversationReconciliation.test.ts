...
describe("conversation stream reconciliation", () => {
  test("ignores stale revisions within one subscription", () => {
    const first = acceptConversationEnvelope(EMPTY_CONVERSATION_STREAM_CURSOR, {
      requestId: "stream-a",
  ...
  test("empty and null snapshots explicitly clear events and current Activity", () => {
    ...
    const empty = reconcileConversationSnapshot(
      previous,
      {
        available: true,
        session_id: "thread-a",
