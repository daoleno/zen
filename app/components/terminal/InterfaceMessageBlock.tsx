...
interface InterfaceMessageBlockProps {
  block: MessageBlock;
  ...
  const tableHorizontalGesture = useMemo(
    () =>
      Gesture.Simultaneous(
        Gesture.Pan().activeOffsetX([-6, 6]).failOffsetY([-8, 8]),
        Gesture.Native(),
      ),
    [],
  );
          ...
          {block.items.map((item, itemIndex) => (
            <View
              ...
              style={[
                ...
                item.depth > 0
                  ? { paddingLeft: listItemIndent(item.depth) }
                  : null,
              ]}
            >
              <View
