...
} from "./InterfaceNativeMarkdownBodyModel";
import { TimelineTextSelectableContext } from "./TimelineTextSelectableContext";

interface InterfaceNativeMarkdownBodyProps {
  value: string;
...
export function InterfaceNativeMarkdownBody({
  value,
  chrome,
  theme,
  compact = false,
  streaming = false,
  renderFallback,
}: InterfaceNativeMarkdownBodyProps) {
  const {
    selectable: textSelectable,
    onTextSelectionGestureStart,
    onTextSelectionGestureEnd,
  } = useContext(TimelineTextSelectableContext);
  const selectionStartTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  const selectionFreezeActiveRef = useRef(false);
  const markdown = useMemo(
    () => prepareInterfaceMarkdown(value, streaming),
    [streaming, value],
  );
  const markdownStyle = useMemo(
    () => interfaceMarkdownStyle(chrome, theme, compact),
  ...
  }, [clearSelectionStartTimer, onTextSelectionGestureStart]);
