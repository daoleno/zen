...
export function acceptConversationEnvelope(
  ...
  const newerGeneration =
    typeof envelopeGeneration === "number" &&
    (typeof currentGeneration !== "number" || envelopeGeneration > currentGeneration);
  if (
    !newerGeneration &&
    typeof currentGeneration === "number" &&
    typeof envelopeGeneration === "number" &&
  ...
  if (!newerGeneration && sameRequest && envelope.revision > 0 && envelope.revision <= current.revision) {
    return {
      accepted: false,
      sameConversation: conversationIdsMatch(current.conversationId, conversationId),
      cursor: current,
