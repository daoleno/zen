...
import { codexChatSessionCacheKey } from "./codexChatSessionIdentity";
...
export type PendingUserMessageInput = Omit<
  PendingUserMessage,
  "id" | "createdAt" | "createdAfterMaxSeq" | "createdAfterEventIds"
>;
...
export function codexChatThreadReducer(
  ...
  switch (action.type) {
    ...
    case "stream_start":
      return {
        ...
        error: null,
        pendingUserMessages:
          typeof state.streamCursor.generation === "number"
            ? []
...
function applyIncomingConversation(
  ...
  const pendingUserMessages = reconcilePendingUserMessages(
    state.pendingUserMessages,
    nextConversation,
  );
  if (
