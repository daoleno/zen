...
const barSource = readFileSync(
  join(import.meta.dir, "WorkerSessionSelectionBar.tsx"),
  "utf8",
);
const rowSource = readFileSync(
  join(import.meta.dir, "WorkerSessionRow.tsx"),
  "utf8",
);
const containerSource = readFileSync(
  join(import.meta.dir, "WorkerListRowContainer.tsx"),
  "utf8",
);
...
describe("WorkerSessionSelectionBar chrome", () => {
...
describe("WorkerSessionRow selection contract", () => {
...
describe("WorkerListRowContainer selection contract", () => {
  ...
  test("press opens the Session in normal mode and toggles in selection mode", () => {
    expect(containerSource).toContain("onOpenWorker(agent)");
...
describe("Sessions list integration contract", () => {
  ...
  test("selection survives reorder by stable key and prunes on disappearance", () => {
    ...
    expect(listSource).toContain("authoritativeWorkerKeySet");
    expect(listSource).toContain("batch.settleDisappeared");
  });

  test("batch submission uses current-server selected Sessions, not sorted viewport rows", () => {
    expect(listSource).toContain(
      "displayWorkers.filter((agent) => selectedKeys.has(agent.key))",
    );
    expect(listSource).not.toContain(
      "sortedWorkers.filter((agent) => selectedKeys.has(agent.key))",
    );
  });

  test("one confirmation with the exact count, exact IDs submitted", () => {
    expect(listSource).toContain("sessionTerminationConfirmMessage(count)");
    expect(listSource).toContain("createSessionTerminationEntries");
    expect(listSource).toContain("serverId: agent.serverId");
    expect(listSource).toContain("workerId: agent.id");
  });

  test("duplicate submission is prevented while a batch runs", () => {
    expect(listSource).toContain("if (terminationRunning || selectedWorkers.length === 0)");
  ...
  test("the row context menu long-press surface is gone", () => {
    ...
    expect(listSource).not.toContain("handleTerminateWorker");
  });
  ...
  test("selection chrome replaces the app bar while active", () => {
    ...
    expect(listSource).toContain("<WorkerSessionSelectionBar");
  });
});
